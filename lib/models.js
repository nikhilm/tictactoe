window.GridModel = Backbone.Model.extend({
    ...
    initialize: function() {
        console.log(this.cid, 'grid', this.get('grid'));
        window.subscriber.bind('move', this.opponentMove, this);
    },

    occupied: function(row, col) {
        return this.get('grid')[row*3+col] != null;
    },
    ...
    opponentMove: function(move) {
        if (this.get('gameover'))
            return;
        if (this.get('ourTurn') || this.occupied(move.row, move.col)) {
